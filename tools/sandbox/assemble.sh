...
clips={c['shot']:c for c in json.load(open('clips.json')) if c.get('url')}
...
ffmpeg -y -loglevel error -i video.mp4 -ss $T0 -t $DUR -i vo.wav -vf "ass=captions.ass" -map 0:v -map 1:a -c:v libx264 -preset veryfast -crf 20 -pix_fmt yuv420p -c:a aac -b:a 160k -shortest "$OUT"
