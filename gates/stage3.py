...
tts = sc.split("## Текст для TTS")[-1].split("## Источники")[0] if "## Текст для TTS" in sc else sc
words = len(re.findall(r"[\w'’ʼ-]+", re.sub(r"\[[^\]]*\]|#.*|\*\*|F-\d+", " ", tts)))
...
hits = [t for t in bl if t not in fl and re.search(r"(?<!\w)" + re.escape(t) + r"(?!\w)", (sc + json.dumps(cards, ensure_ascii=False)).lower())]  # TRANSPLANT описывает донора — не сканируем
...
ss = d.get("sale_starts_s"); cs = next((c.get("t_start") for c in cards if c.get("product_visible") and c.get("t_start") is not None), None)
ch.append(("G3-8", cs is not None and ss and within(cs, ss, 5), f"начало продажи: клон {cs} vs ДНК {ss} (±5%)"))
roles = {c.get("role") for c in cards} | {f for c in cards for f in c.get("funcs", [])}; need = {"hook","proof","transformation","price","urgency","callback","cta"}
